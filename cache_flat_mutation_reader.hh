...
class cache_flat_mutation_reader final : public flat_mutation_reader::impl {
    ...
    // Underlying reader conforming to _schema.
    std::optional<flat_mutation_reader> _underlying;
...
future<> cache_flat_mutation_reader::ensure_underlying(db::timeout_clock::time_point timeout) {
    ...
    return _read_context->ensure_underlying(timeout).then([this, timeout] {
        _underlying = make_delegating_reader(_read_context->underlying().underlying());
        _underlying->upgrade_schema(_schema);
    });
}
