...
async def test_tablet_metadata_propagates_with_schema_changes_in_snapshot_mode(manager: ManagerClient):
    ...
    keys = range(10)
    await asyncio.gather(*[manager.cql.run_async(f"INSERT INTO test.test (pk, c) VALUES ({k}, 1);", execution_profile='whitelist')
                           for k in keys])
    ...
    await manager.server_start(s0, wait_for_others=True, others_count=2)
    ...
    for s in servers:
        manager.server_restart(s, wait_for_others=True, others_count=2)
