...
static future<> test_huge_tablet_metadata(app_template& app) {
    return do_with_cql_env_thread([&] (cql_test_env& e) {
        ...
        auto mem_before = seastar::memory::stats().allocated_memory();

        for (int i = 0; i < nr_tables; ++i) {
            tablet_map tmap(tablets_per_table);

            for (int j = 0; j < tmap.tablet_count(); ++j) {
                aborted.check();
                thread::maybe_yield();
        ...
        auto mem_after = seastar::memory::stats().allocated_memory();

        testlog.info("Memory usage increased by {} KiB for {} tablets", (mem_after - mem_before) / 1024, total_tablets);
...
int scylla_tablets_main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&] {
        return seastar::async([&] {
            ...
            try {
                test_huge_tablet_metadata(app).get();
