...
SEASTAR_TEST_CASE(test_v2_merging_in_evictable_snapshot) {
    return seastar::async([] {
        ...
        // This hopefully generates some non-dummy entries with continuity flag not set.
        ...
        if (!succeeded) {
            testlog.warn("m1_v2 has no non-dummy entries with continuity=false");
        }
        ...
        auto insert = [&] (mutation_partition_v2& p) {
            for (rows_entry& e : p.mutable_clustered_rows()) {
                tracker.insert(e);
            }
        };
        ...
        auto drop_entries = defer([&] {
            ...
            m1_v2 = mutation_partition_v2(s.shared_from_this());
            m2_v2 = mutation_partition_v2(s.shared_from_this());
        });
...
SEASTAR_TEST_CASE(test_mutation_partition_conversion_between_v2_and_v1) {
    return seastar::async([] {
        ...
        assert_that(v1_from_v2).is_equal_to(m);
    });
}
