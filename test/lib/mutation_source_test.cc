...
public:
    ...
    mutation operator()() {
        ...
        auto set_random_cells = [&] (row& r, column_kind kind) {
            const auto kind_column_count = _schema->columns_count(kind);
            if (kind_column_count == 0) {
                return;
            }
            std::uniform_int_distribution<column_id> column_count_dist(1, kind_column_count);
            std::uniform_int_distribution<column_id> column_id_dist(0, kind_column_count - 1);
