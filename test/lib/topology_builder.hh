...
///
/// As long as this instance is alive, topology state machine is locked so there are no concurrent topology operations
/// and load balancing.
///
    ...
    void unlock_topology() {
        ...
        while (true) {
            ...
            service::topology_change change({service::topology_mutation_builder(guard.write_timestamp())
                                                 ...
                                                 .build()});
            service::group0_command g0_cmd = client.prepare_command(std::move(change), guard, "unlocking topology");
            try {
                client.add_entry(std::move(g0_cmd), std::move(guard), as).get();
            } catch (service::group0_concurrent_modification&) {
                testlog.info("Concurrent modification detected while unlocking topology, retrying");
            }
        }
    }
public:
                              ...
                              std::optional<endpoint_dc_rack> rack_override = {})
    {
        ...
        std::unordered_set<dht::token> tokens({new_token()});
    ...
    void set_node_state(locator::host_id id, service::node_state state) {
        ...
        builder.with_node(raft::server_id(id.uuid()))
                .set("node_state", state);
        service::topology_change change({builder.build()});
        service::group0_command g0_cmd = client.prepare_command(std::move(change), guard, format("node {} state={}", id, state));
