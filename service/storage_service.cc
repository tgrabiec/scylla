...
future<std::unordered_map<dht::token_range, inet_address_vector_replica_set>>
storage_service::get_range_to_address_map(locator::token_effective_replication_map_ptr erm) const {
...
future<std::unordered_map<dht::token_range, inet_address_vector_replica_set>>
storage_service::get_range_to_address_map(locator::token_effective_replication_map_ptr erm,
...
future<> storage_service::replicate_to_all_cores(mutable_token_metadata_ptr tmptr) noexcept {
    ...
    std::vector<std::unordered_map<sstring, locator::token_effective_replication_map_ptr>> pending_effective_replication_maps;
    ...
    try {
        ...
        co_await container().invoke_on_all([&] (storage_service& ss) {
            ...
            for (auto&& [id, cf] : db.get_column_families()) { // Safe because we iterate without preemption
                ...
                if (rs->is_per_table()) {
                    erm = dynamic_cast<const locator::per_table_replication_strategy&>(*rs)
                            .make_replication_map(id, tmptr);
                } else {
...
future<std::map<gms::inet_address, float>> storage_service::effective_ownership(sstring keyspace_name) {
    return run_with_no_api_lock([keyspace_name] (storage_service& ss) mutable -> future<std::map<gms::inet_address, float>> {
        locator::token_effective_replication_map_ptr erm;
        if (keyspace_name != "") {
...
storage_service::get_changed_ranges_for_leaving(locator::token_effective_replication_map_ptr erm, inet_address endpoint) {
...
storage_service::get_new_source_ranges(locator::token_effective_replication_map_ptr erm, const dht::token_range_vector& ranges) const {
    auto my_address = get_broadcast_address();
...
storage_service::get_ranges_for_endpoint(const locator::token_effective_replication_map_ptr& erm, const gms::inet_address& ep) const {
