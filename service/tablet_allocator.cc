...
class load_balancer {
    ...
    struct node_load {
        ...
        // The maximum per-shard load in terms of tablet count after the node would be internally balanced.
        load_type avg_load = 0;

        std::vector<shard_id> candidates; // heap which tracks most-loaded shards using candidates_cmp().
        std::vector<shard_load> shards;

        // Call when tablet_count changes.
        void update() {
            avg_load = get_balanced_max_load(tablet_count);
        }

        load_type get_balanced_max_load(uint64_t tablets) const {
...
public:
    ...
    future<migration_plan> make_plan() {
        ...
        for (auto&& dc : topo.get_datacenters()) {
            ...
            std::copy(dc_plan.begin(), dc_plan.end(), std::back_inserter(plan));
        }
    ...
    future<migration_plan> make_plan(sstring dc) {
        ...
        /// per-shard load. If we achieve balance according to this metric, and then reblance the nodes internally,
        ...
        ///
        /// We can adjust the algorithm to work with multiple targets at a time. We should keep the candidate exclusion set
        /// per-target.
                ...
                for (auto&& replica : ti.replicas) {
                    if (nodes.contains(replica.host)) {
                        ...
                        if (replica.shard >= nodes[replica.host].shard_count) {
                            ...
                            throw std::runtime_error(format("Tablet {} replica {} targets non-existent shard", gtid, replica));
                        }
                    }
                }
            ...
            for (const tablet_info& ti : tmap.tablets()) {
                ...
                for (auto&& replica : ti.replicas) {
                    ...
                    if (shard_load_info.tablet_count == 0) {
                        node_load_info.candidates.push_back(replica.shard);
                    }
        ...
        std::vector<host_id> candidate_nodes;
        candidate_nodes.reserve(nodes.size());
        ...
        for (auto&& [host, node_load] : nodes) {
            ...
            candidate_nodes.push_back(host);
            std::make_heap(node_load.candidates.begin(), node_load.candidates.end(), node_load.candidates_cmp());
        }

        std::make_heap(candidate_nodes.begin(), candidate_nodes.end(), nodes_cmp);
        ...
        while (plan.size() < batch_size && !candidate_nodes.empty()) {
            co_await coroutine::maybe_yield();

            std::pop_heap(candidate_nodes.begin(), candidate_nodes.end(), nodes_cmp);
            auto src_host = candidate_nodes.back();
            auto& src_node_info = nodes[src_host];

            if (std::max(max_off_candidate_load, src_node_info.avg_load) == nodes[target].avg_load) {
            ...
            if (src_node_info.avg_load <= nodes[target].avg_load) {
                lblogger.debug("No more candidate nodes");
                break;
            }

            // Prevent load inversion which can lead to oscillations.
            if (src_node_info.get_balanced_max_load(nodes[src_host].tablet_count - 1) <
                    nodes[target].get_balanced_max_load(nodes[target].tablet_count + 1)) {
                lblogger.debug("No more candidate nodes");
                break;
            }

            if (src_node_info.candidates.empty()) {
            ...
            auto push_back_node_candidate = seastar::defer([&] {
                std::push_heap(candidate_nodes.begin(), candidate_nodes.end(), nodes_cmp);
            });

            std::pop_heap(src_node_info.candidates.begin(), src_node_info.candidates.end(), src_node_info.candidates_cmp());
            auto src_shard = src_node_info.candidates.back();
            ...
            if (src_shard_info.candidates.empty()) {
                ...
                src_node_info.candidates.pop_back();
                continue;
            }
            auto push_back_shard_candidate = seastar::defer([&] {
                std::push_heap(src_node_info.candidates.begin(), src_node_info.candidates.end(), src_node_info.candidates_cmp());
            });
            ...
            for (auto&& r : tmap.get_tablet_info(source_tablet.tablet).replicas) {
                if (r.host == target) {
                    lblogger.debug("candidate tablet {} skipped because it has a replica on target node", source_tablet, target);
        ...
        if (plan.empty()) {
            ...
            lblogger.warn("Not possible to achieve balance.");
        }
