...
void mutation_partition::set_continuity(const schema& s, const position_range& pr, is_continuous cont) {
    ...
    if (end == _rows.end() || less(pr.end(), end->position())) {
        end = _rows.insert(end, *current_allocator().construct<rows_entry>(s, pr.end(), is_dummy::yes,
            end == _rows.end() ? is_continuous::yes : end->continuous()), less);
    }

    auto i = _rows.lower_bound(pr.start(), less);
    if (less(pr.start(), i->position())) {
        i = _rows.insert(i, *current_allocator().construct<rows_entry>(s, pr.start(), is_dummy::yes, i->continuous()), less);
    }
