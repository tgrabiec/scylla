    ...
    auto apply_tombstone_to_rows = [&] (apply_resume::stage stage, tombstone tomb, rows_type::iterator i, rows_type::iterator end) -> stop_iteration {
        ...
        while (i != end) {
            ...
            if (need_preempt() && i != end) {
                res = apply_resume(stage, position_in_partition(i->position()));
    ...
    return with_allocator(region.allocator(), [&] {
        {
            ...
            try {
                return _worker_state->alloc_section(region, [&] {
                    ssize_t dirty_before = region.occupancy().total_space();
                    auto res = snp.merge_partition_versions(_app_stats);
                    ssize_t freed_space = dirty_before - region.occupancy().total_space();
                    if (_on_space_freed && freed_space > 0) {
                        _on_space_freed(freed_space);
                    }
                    return res;
                });
            } catch (...) {
