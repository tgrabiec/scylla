...
locator::token_effective_replication_map_ptr keyspace::get_effective_replication_map() const {
...
void database::add_column_family(keyspace& ks, schema_ptr schema, column_family::config cfg) {
    ...
    if (rs.is_per_table()) {
        erm = dynamic_cast<const locator::per_table_replication_strategy&>(rs)
                .make_replication_map(schema->id(), _shared_token_metadata.get());
    } else {
...
std::unordered_map<sstring, locator::token_effective_replication_map_ptr> database::get_non_local_strategy_keyspaces_erms() const {
...
void
keyspace::update_effective_replication_map(locator::token_effective_replication_map_ptr erm) {
