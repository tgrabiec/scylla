...
public:
    table(schema_ptr schema, config cfg, lw_shared_ptr<const storage_options> sopts, db::commitlog& cl, compaction_manager& cm, sstables::sstables_manager& sm, cell_locker_stats& cl_stats, cache_tracker& row_cache_tracker, locator::effective_replication_map_ptr erm = nullptr)
        : table(schema, std::move(cfg), std::move(sopts), &cl, cm, sm, cl_stats, row_cache_tracker, std::move(erm)) {}
    table(schema_ptr schema, config cfg, lw_shared_ptr<const storage_options> sopts, no_commitlog, compaction_manager& cm, sstables::sstables_manager& sm, cell_locker_stats& cl_stats, cache_tracker& row_cache_tracker, locator::effective_replication_map_ptr erm = nullptr)
...
private:
    ...
    locator::token_effective_replication_map_ptr _effective_replication_map;
...
public:
    ...
    void update_effective_replication_map(locator::token_effective_replication_map_ptr erm);
    ...
    locator::token_effective_replication_map_ptr get_effective_replication_map() const;
...
public:
    ...
    std::unordered_map<sstring, locator::token_effective_replication_map_ptr> get_non_local_strategy_keyspaces_erms() const;
