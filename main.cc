    ...
    return app.run(ac, av, [&] () -> future<int> {
        ...
        return seastar::async([&app, cfg, ext, &cm, &db, &qp, &bm, &proxy, &forward_service, &mm, &mm_notifier, &ctx, &opts, &dirs,
          ...
          try {
            ...
            the_tablet_allocator.start(std::ref(mm_notifier), std::ref(db)).get();
            auto stop_tablet_allocator = defer_verbose_shutdown("tablet allocator", [&the_tablet_allocator] {
            ...
            if (raft_gr.local().is_enabled()) {
                ...
                if (cfg->check_experimental(db::experimental_features_t::feature::TABLETS)) {
                    startlog.error("Bad configuration: RAFT feature has to be enabled if TABLETS is enabled");
                    throw bad_configuration_error();
                }
            }
