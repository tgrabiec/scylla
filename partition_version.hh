...
// overlapping, with exceptions for points corresponding to complete rows. A row
// may overlap with another row. A later version may have a row which falls into
// a continuous interval in the older version. A newer version cannot have a
// continuous interval which is not a row and covers a row in the older version.
// We make use of this assumption to make calculation of the union of intervals
// on merging easier.
//
// When marking a range as continuous with some rows present only in older
// versions, dummy entries are inserted in the latest version, which marks ranges
// corresponding to those rows as discontinuous. When merged with older versions,
// the complete row jumps into the gap represented by a dummy entry and it would
// form a fully continuous range. It would be also correct to copy the rows from
// older versions into the latest version, but less efficient.
//
