...
static void log_schema_diff(const std::vector<mutation>& mutations) {
    ...
    try {
        struct key_with_schema {
            dht::decorated_key dk;
            schema_ptr s;
            bool operator<(const key_with_schema& other) const {
                return dk.less_compare(*s, other.dk);
            }
        };
        using mutation_key = std::pair<utils::UUID, key_with_schema>;
        auto make_map = [] (std::vector<mutation> muts) {
            std::map<mutation_key, mutation> map;
            for (auto&& mut : muts) {
                auto key = std::make_pair(mut.column_family_id(), key_with_schema{mut.decorated_key(), mut.schema()});
                map.emplace(std::move(key), std::move(mut));
            }
            return map;
        };
        auto theirs = make_map(mutations);
        auto ours = make_map(read_local_schema_mutations(schema_features::full()).get0());
        map_difference<mutation_key> diff = difference(theirs, ours);
        bool diff_found = false;
        for (auto&& id : diff.entries_only_on_right) {
            diff_logger.trace("only on this node: {}", ours.at(id));
            diff_found = true;
        }
        for (auto&& id : diff.entries_differing) {
            ...
            diff_logger.trace("differing: {{{}.{} {} {}}}", s->ks_name(), s->cf_name(), ours_m.decorated_key(),
                mutation_partition::printer(*s, p_diff));
            diff_found = true;
        }
        if (!diff_found) {
            diff_logger.trace("no extra writes on this node");
        }
    } catch (...) {
