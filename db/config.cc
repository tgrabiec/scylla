...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , enable_single_row_partition(this, "enable_single_row_partition", liveness::MustRestart, value_status::Used, true,
        "Use the optimized single_row_partition in-memory storage format in the row cache and memtables for tables without clustering columns. When disabled, such tables use the generic partition storage format. Requires a restart to take effect.")
