...
//    mplog.trace("apply {}\nto: {}", mutation_partition_v2::printer(s, p), mutation_partition_v2::printer(s, *this));
    ...
    while (p_i != p._rows.end()) {
        ...
        // We cannot uncomment the more efficient version below before changing partition_snapshot::squashed() to propagate
        ...
        if (miss) {
            ...
            if (i != _rows.end() && i->continuous()) {
                ...
                if (!src_e.continuous() && src_e.range_tombstone() > i->range_tombstone()) {
                    if (src_e.dummy()) {
                        lb_i->set_range_tombstone(i->range_tombstone());
                    } else {
