...
public:
    ...
    virtual inet_address_vector_replica_set get_natural_endpoints(const token& search_token, const token_effective_replication_map& erm) const;
    // Returns the last stop_iteration result of the called func
    virtual stop_iteration for_each_natural_endpoint_until(const token& search_token, const token_effective_replication_map& erm, const noncopyable_function<stop_iteration(const inet_address&)>& func) const;
...
using token_effective_replication_map_ptr = shared_ptr<const token_effective_replication_map>;
...
class global_token_effective_replication_map {
    std::vector<foreign_ptr<token_erm_ptr>> _erms;

public:
    global_token_effective_replication_map() : _erms(smp::count) {}
    global_token_effective_replication_map(global_token_effective_replication_map&&) = default;
    global_token_effective_replication_map& operator=(global_token_effective_replication_map&&) = default;

    future<> get_keyspace_erms(sharded<replica::database>& sharded_db, std::string_view keyspace_name);

    const token_effective_replication_map& get() const noexcept {
        return *_erms[this_shard_id()];
    }

    const token_effective_replication_map& operator*() const noexcept {
        return get();
    }
...
template <>
struct fmt::formatter<locator::token_effective_replication_map::factory_key> {
    constexpr auto parse(format_parse_context& ctx) {
        return ctx.end();
    }

    template <typename FormatContext>
    auto format(const locator::token_effective_replication_map::factory_key& key, FormatContext& ctx) {
...
template<>
struct appending_hash<locator::token_effective_replication_map::factory_key> {
    template<typename Hasher>
    void operator()(Hasher& h, const locator::token_effective_replication_map::factory_key& key) const {
...
template <>
struct hash<locator::token_effective_replication_map::factory_key> {
    size_t operator()(const locator::token_effective_replication_map::factory_key& key) const {
        factory_key_hasher h;
        appending_hash<locator::token_effective_replication_map::factory_key>{}(h, key);
        return h.finalize();
    }
};
...
class effective_replication_map_factory : public peering_sharded_service<effective_replication_map_factory> {
    std::unordered_map<token_effective_replication_map::factory_key, token_effective_replication_map*> _effective_replication_maps;
    future<> _background_work = make_ready_future<>();
    bool _stopped = false;

public:
    // looks up the token_effective_replication_map on the local shard.
...
private:
    token_erm_ptr find_effective_replication_map(const token_effective_replication_map::factory_key& key) const;
    token_erm_ptr insert_effective_replication_map(mutable_token_erm_ptr erm, token_effective_replication_map::factory_key key);

    bool erase_effective_replication_map(token_effective_replication_map* erm);
