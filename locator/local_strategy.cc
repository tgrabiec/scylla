...
inet_address_vector_replica_set local_strategy::get_natural_endpoints(const token&, const token_effective_replication_map&) const {
    return inet_address_vector_replica_set({utils::fb_utilities::get_broadcast_address()});
}

stop_iteration local_strategy::for_each_natural_endpoint_until(const token&, const token_effective_replication_map&, const noncopyable_function<stop_iteration(const inet_address&)>& func) const {
