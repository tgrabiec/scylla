...
inet_address_vector_replica_set everywhere_replication_strategy::get_natural_endpoints(const token&, const token_effective_replication_map& erm) const {
...
stop_iteration everywhere_replication_strategy::for_each_natural_endpoint_until(const token&, const token_effective_replication_map& erm, const noncopyable_function<stop_iteration(const inet_address&)>& func) const {
    const auto& tm = *erm.get_token_metadata_ptr();
    if (tm.sorted_tokens().empty()) {
