...
inet_address_vector_replica_set abstract_replication_strategy::get_natural_endpoints(const token& search_token, const token_effective_replication_map& erm) const {
    const token& key_token = erm.get_token_metadata_ptr()->first_token(search_token);
    auto res = erm.get_replication_map().find(key_token);
    return res->second;
}

stop_iteration abstract_replication_strategy::for_each_natural_endpoint_until(const token& search_token, const token_effective_replication_map& erm, const noncopyable_function<stop_iteration(const inet_address&)>& func) const {
    const token& key_token = erm.get_token_metadata_ptr()->first_token(search_token);
    auto res = erm.get_replication_map().find(key_token);
...
inet_address_vector_replica_set token_effective_replication_map::get_natural_endpoints_without_node_being_replaced(const token& search_token) const {
...
inet_address_vector_topology_change token_effective_replication_map::get_pending_endpoints(const token& search_token, const sstring& ks_name) const {
...
token_effective_replication_map::do_get_ranges(noncopyable_function<stop_iteration(bool&, const inet_address&)> consider_range_for_endpoint) const {
    dht::token_range_vector ret;
...
dht::token_range_vector
token_effective_replication_map::get_ranges(inet_address ep) const {
...
dht::token_range_vector
token_effective_replication_map::get_primary_ranges(inet_address ep) const {
    // The callback function below is called for each endpoint
    // in each token natural endpoints.
...
dht::token_range_vector
token_effective_replication_map::get_primary_ranges_within_dc(inet_address ep) const {
...
token_effective_replication_map::get_range_addresses() const {
...
future<replication_map> token_effective_replication_map::clone_endpoints_gently() const {
...
inet_address_vector_replica_set token_effective_replication_map::get_natural_endpoints(const token& search_token) const {
    return _rs->get_natural_endpoints(search_token, *this);
}

stop_iteration token_effective_replication_map::for_each_natural_endpoint_until(const token& search_token, const noncopyable_function<stop_iteration(const inet_address&)>& func) const {
    return _rs->for_each_natural_endpoint_until(search_token, *this, func);
}

future<> token_effective_replication_map::clear_gently() noexcept {
    co_await utils::clear_gently(_replication_map);
    co_await utils::clear_gently(_tmptr);
}

token_effective_replication_map::~token_effective_replication_map() {
    if (is_registered()) {
...
future<token_effective_replication_map_ptr> effective_replication_map_factory::create_effective_replication_map(replication_strategy_ptr rs, token_metadata_ptr tmptr) {
    ...
    auto ref_erm = co_await container().invoke_on(0, [key] (effective_replication_map_factory& ermf) -> future<foreign_ptr<token_effective_replication_map_ptr>> {
        auto erm = ermf.find_effective_replication_map(key);
        co_return make_foreign<token_effective_replication_map_ptr>(std::move(erm));
    });
    mutable_token_effective_replication_map_ptr new_erm;
    if (ref_erm) {
...
token_effective_replication_map_ptr effective_replication_map_factory::find_effective_replication_map(const token_effective_replication_map::factory_key& key) const {
...
token_effective_replication_map_ptr effective_replication_map_factory::insert_effective_replication_map(mutable_token_effective_replication_map_ptr erm, token_effective_replication_map::factory_key key) {
...
bool effective_replication_map_factory::erase_effective_replication_map(token_effective_replication_map* erm) {
...
future<> global_token_effective_replication_map::get_keyspace_erms(sharded<replica::database>& sharded_db, std::string_view keyspace_name) {
...
future<global_token_effective_replication_map> make_global_effective_replication_map(sharded<replica::database>& sharded_db, std::string_view keyspace_name) {
    global_token_effective_replication_map ret;
...
std::ostream& operator<<(std::ostream& os, const locator::token_effective_replication_map::factory_key& key) {
    os << key.rs_type;
    os << '.' << key.ring_version;
    char sep = ':';
