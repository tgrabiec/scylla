...
public:
    ...
    // Returns change of value over time, assuming last time the value was checked was "duration" ago.
    //template<typename Rep, typename Period, typename Precision = float>
    //Precision rate(std::chrono::duration<Rep, Period> duration) {
    //    return Precision(change()) / std::chrono::duration<Precision>(duration).count();
    //}
};
