...
using generator_fn = std::function<stdx::optional<mutation>()>;
...
public:
    ...
    // create_table_statement should be a sprint() pattern with %s in place of the table name
    dataset(const std::string& name, const std::string& message, const std::string& create_table_statement)
            : _name(name)
            , _message(message)
            , _create_table_statement(create_table_statement)
            , _table_name([&name] {
                std::string r(name);
...
std::unique_ptr<dataset_acceptor> make_test_fn(void (*fn)(column_family&, DataSet&)) {
    return std::unique_ptr<dataset_acceptor>(
        std::make_unique<dataset_acceptor_impl<DataSet>>(fn).release());
}
...
public:
    ...
    void write_dataset_population(const dataset& ds) override {
        write_common_test_group(sprint("population-%s", ds.name()), ds.description(), ds);
    }
...
public:
    simple_large_part_ds(std::string name, std::string desc) : dataset(name, desc,
        "create table %s (pk int, ck int, value blob, primary key (pk, ck))") {}
...
public:
    small_part_ds1() : dataset("small-part", "Many small partitions with no clustering key",
        "create table %s (pk int, value blob, primary key (pk))") {}
...
void populate(const std::vector<dataset*>& datasets, cql_test_env& env, const table_config& cfg, size_t flush_threshold) {
    ...
    for (dataset* ds_ptr : datasets) {
        ...
        env.execute_cql(sprint(ds.create_table_statement()
                               + (cfg.with_compression ? " WITH compression = { 'sstable_compression': 'LZ4Compressor' };"
