...
class memtable_snapshot_source {
    std::vector<lw_shared_ptr<memtable>> _memtables;
public:
    void clear() {
        _memtables.clear();
    }
    void apply(memtable& mt) {
        auto new_mt = make_lw_shared<memtable>(mt.schema());
        new_mt->apply(mt).get();
        _memtables.push_back(std::move(new_mt));
    }
...
SEASTAR_TEST_CASE(test_cache_population_and_update_race) {
    return seastar::async([] {
        auto s = make_schema();
        memtable_snapshot_source memtables;
        throttle thr;
