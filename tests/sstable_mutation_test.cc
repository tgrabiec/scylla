...
SEASTAR_TEST_CASE(test_index_and_variable_partition_key_size) {
    return seastar::async([] {
        storage_service_for_tests ssft;
        auto dir = make_lw_shared<tmpdir>();
        ...
        auto make_pkey_text = [] (size_t pad_size) {
            static int i = 0;
            return sstring(sprint("pkey_%x_", i++)) + sstring(sstring::initialized_later(), pad_size);
        };
        ...
        for (auto&& key : small_keys) {
            mutation m(s, key);
            auto value = sstring(sstring::initialized_later(), 128);
        ...
        std::cout << "took " << std::chrono::duration_cast<std::chrono::microseconds>(duration).count() << " us\n";

        std::cout << sizeof(index_entry) << "\n";
